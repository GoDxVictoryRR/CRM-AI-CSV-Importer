...
import { computeCsvHash } from '../src/services/cacheService';
import { CrmRecord, SkippedRecord, ImportResponse } from '../src/types/crm';

// ─── Test Harness ───────────────────────────────────────────────────────────
// Expose the private validateAndCollect method for direct unit testing
// without needing to mock a full Gemini API roundtrip.
class TestableAiExtractionService extends AiExtractionService {
  public testValidateAndCollect(
    ...
    return (this as any).validateAndCollect(rawRow, rowIndex, extracted, parsedList, skippedList);
  }
}

// Minimal stub AIProvider that is never called in these unit tests
const stubProvider: any = { extractBatch: jest.fn() };
const service = new TestableAiExtractionService(stubProvider);

function run(
  extracted: CrmRecord | null,
  rawRow: Record<string, string> = {}
): { parsed: CrmRecord[]; skipped: SkippedRecord[] } {
...
describe('F7 crm_status enum sanitization', () => {
  ...
  test('clears invalid crm_status to empty string', () => {
    const { parsed } = run({ email: 'a@b.com', crm_status: 'INTERESTED' as any });
    expect(parsed[0].crm_status).toBe('');
  });

  test('is case-insensitive for valid statuses', () => {
    const { parsed } = run({ email: 'a@b.com', crm_status: 'bad_lead' as any });
    expect(parsed[0].crm_status).toBe('BAD_LEAD');
  });

  test('clears completely unknown junk status', () => {
    const { parsed } = run({ email: 'a@b.com', crm_status: 'HOT_PROSPECT' as any });
    expect(parsed[0].crm_status).toBe('');
  });
});
...
describe('F7 data_source enum sanitization', () => {
  ...
  test('clears invalid data_source to empty string', () => {
    const { parsed } = run({ email: 'a@b.com', data_source: 'facebook' as any });
    expect(parsed[0].data_source).toBe('');
  });

  test('is case-insensitive for valid sources', () => {
    const { parsed } = run({ email: 'a@b.com', data_source: 'EDEN_PARK' as any });
...
describe('P1 computeCsvHash consistency', () => {
  ...
  test('same content produces same hash', () => {
    expect(computeCsvHash(headers, rows)).toBe(computeCsvHash(headers, rows));
  });

  test('hash is consistent across multiple calls', () => {
    const h1 = computeCsvHash(headers, rows);
    const h2 = computeCsvHash(headers, rows);
    const h3 = computeCsvHash(headers, rows);
    expect(h1).toBe(h2);
    expect(h2).toBe(h3);
  });
  ...
  test('headers with different casing produce the same hash (normalized)', () => {
    ...
    expect(computeCsvHash(uppercaseHeaders, uppercaseRows)).toBe(computeCsvHash(headers, rows));
  });
  ...
  test('different content produces different hash', () => {
    ...
    expect(computeCsvHash(headers, altRows)).not.toBe(computeCsvHash(headers, rows));
  });
  ...
  test('extra whitespace in values is normalized (same hash)', () => {
    ...
    expect(computeCsvHash(headers, spacedRows)).toBe(computeCsvHash(headers, rows));
  });

  test('returns a 64-char hex SHA-256 string', () => {
    const hash = computeCsvHash(headers, rows);
